...
namespace CodeWalker.GameFiles
{
    [TC(typeof(EXP))] public class FxcFile : PackedFile
    {
        ...
        public void Load(byte[] data, RpfFileEntry entry)
        {
            ...
            foreach (FxcCBuffer cbuf in CBufferDict.Values)
            {
        ...
        public void WriteXml(StringBuilder sb, int indent, string csofolder)
        {
            ...
            {
                FxcShaderGroup group = ShaderGroups[i];
                string typen = group.Type.ToString() + "s";
                string tagn = typen;
                if (group.OffsetBy1)
                {
        ...
        public string GetMetaString()
        {
            ...
            if (PresetParams != null)
            {
                ...
                foreach (FxcPresetParam ext in PresetParams)
                {
            ...
            if (ShaderGroups != null)
            {
                ...
                foreach (FxcShaderGroup chunk in ShaderGroups)
                {
            ...
            if (Shaders != null)
            {
                ...
                foreach (FxcShader shader in Shaders)
                {
                    ...
                    if ((shader.Variables != null) && (shader.Variables.Length > 0))
                    {
                        ...
                        foreach (string parm in shader.Variables)
                        {
                            sb.AppendLine("      " + parm);
                        }
                    }
                    if ((shader.Buffers != null) && (shader.Buffers.Length > 0))
                    {
                        ...
                        foreach (FxcShaderBufferRef ext in shader.Buffers)
                        {
                            sb.AppendLine("      " + ext.ToString());
                        }
                    }
            ...
            if (CBuffers1 != null)
            {
                ...
                foreach (FxcCBuffer p in CBuffers1)
                {
            ...
            if (Variables1 != null)
            {
                ...
                foreach (FxcVariable p in Variables1)
                {
                    ...
                    {
                        sb.AppendLine("    (Params)");
                        foreach (FxcVariableParam c in p.Params)
                        {
            ...
            if (CBuffers2 != null)
            {
                ...
                foreach (FxcCBuffer p in CBuffers2)
                {
                    sb.AppendLine("  " + p.ToString());
                }
                sb.AppendLine();
    ...
    [TC(typeof(EXP))] public class FxcVariable : IMetaXmlItem
    {
        ...
        public override string ToString()
        {
            string pstr = (ParamCount > 0) ? (", " + ParamCount.ToString() + " params") : "";
            string vstr = (ValueCount > 0) ? (", " + ValueCount.ToString() + " values") : "";
    ...
    [TC(typeof(EXP))] public class FxcTechnique : IMetaXmlItem
    {
        ...
        public void GetNamesFromIndices(FxcFile fxc)
        {
            ...
            foreach (FxcPass pass in Passes)
            {
